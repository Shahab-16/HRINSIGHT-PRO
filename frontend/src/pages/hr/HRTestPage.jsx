...
const HRTestPage = () => {
  ...
  const { testId } = useParams(); // this is token/testId from the invite
  ...
  const finishTest = async () => {
    try {
      // Example: if you want to store test results against the token/testId,
      // you can POST answers to backend with testId.
      // await axios.post(`${BackendURL}/api/hr/submit-test`, { testId, answers });

      console.log("TestId / token:", testId);
      console.log("Answers:", answers);

      toast.success("Test submitted successfully!");
      // send user back to HR dashboard (or show results)
  ...
  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-gradient-to-br from-indigo-50 to-white flex flex-col items-center">
      <div className="w-full bg-white shadow-sm px-6 md:px-12 py-5 flex justify-between items-center border-b border-gray-200 sticky top-0 z-10">
        <h2 className="text-2xl font-semibold text-gray-800 tracking-tight">HR Diagnostic Test</h2>
        <p className="text-gray-600 font-medium text-sm md:text-base">Question {currentIndex + 1} / {questions.length}</p>
      </div>
      ...
      <div className="flex flex-col items-center justify-center flex-grow w-full px-4 md:px-6 lg:px-8 py-10">
        <div className="w-full max-w-3xl bg-white rounded-2xl shadow-lg p-6 md:p-10 transition-all duration-300">
          <h3 className="text-lg md:text-xl font-semibold text-gray-800 mb-2 text-center">
            <span className="text-sm text-gray-500 italic">({currentQuestion.area})</span>
          </h3>
          <h3 className="text-lg md:text-xl font-semibold text-gray-900 mb-8 text-center">{currentQuestion.question}</h3>
            ...
            {currentQuestion.options.map((opt, idx) => (
              <label
                ...
                className={`flex items-start gap-3 p-4 rounded-lg border cursor-pointer transition-all duration-200 ${
                  answers[currentQuestion.id] === idx ? "border-green-500 bg-green-50" : "border-gray-300 hover:bg-gray-100"
                }`}
              >
                <input
                  type="radio"
          ...
          <div className="flex justify-end mt-10">
            <button onClick={handleSubmit} className="px-8 py-3 font-semibold rounded-md text-white bg-green-600 hover:bg-green-700 transition-all duration-200 shadow-sm">
      ...
      <p className="mt-4 mb-8 text-sm text-gray-500 text-center px-4 max-w-2xl">Answer all questions carefully before submitting. Once submitted, your answers will be automatically evaluated.</p>
    </div>
  );
};
